...
/**
 * 自己实现的一个通用ActivityGroup。  * 可以通过简单的重写它来制作有导航按钮和用导航按钮控制动态加载Activity的ActivityGroup。  * 开发者需要在实现类中实现三个方法：  *     1.指定动态加载Activity的容器的对象，getContainer()方法。  *     2.初始化所有的导航按钮，initRadioBtns()方法，开发者要遍历所有的导航按钮并执行initRadioBtn(int id)方法。  *     3.实现导航按钮动作监听器的具体方法，onCheckedChanged(...)方法。这个方法将实现某个导航按钮与要启动对应的Activity的关联关系，可以调用setContainerView(...)方法。  * @author zet  *
 */
public abstract class AbstractMyActivityGroup extends ActivityGroup implements CompoundButton.OnCheckedChangeListener {
    ...
    abstract protected ViewGroup getContainer(); /**  * 供实现类调用，根据导航按钮id初始化按钮  * @param id  */
    ...
    /**
     * 供开发者在实现类中调用，能将Activity容器内的Activity移除，再将指定的某个Activity加入  * @param activityName 加载的Activity在localActivityManager中的名字  * @param activityClassTye    要加载Activity的类型
     */
