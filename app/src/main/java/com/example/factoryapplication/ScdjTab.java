...
public class ScdjTab extends BaseFragment implements  AdapterView.OnItemClickListener ,OnClickListener {
...
//                if(!isEmpty(khh)) url = url + "&khbh=" +  (khh ==null?"":khh);
//                if(!isEmpty(hth)) url = url + "&htbh=" + hth ;
//                if(!isEmpty(khh)) url = url + "&cpmc=" + cpmc ;
//                if(!isEmpty(khh)) url = url + "&startDate=" + begin ;
//                if(!isEmpty(khh)) url = url + "&endDate=" + end ;
...
//        sendByGet(Constant.BUSINESS_DDXZ + "?status=" + status  );
//        String url = Constant.BUSINESS_DDXZ + "?status=" + status  ;
        String url = Constant.BUSINESS_DDXZ + "?status="+status+"&rybh=" + SPUtils.get(activity,"yhbh","");
...
//        dismissProcess();
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        String gx = SPUtils.get(getActivity(),"ry_scgx","0" ).toString();
        if("1".equals(gx)) {
            DdxzListItemEntity ddxzListItemEntity = (DdxzListItemEntity) parent.getAdapter().getItem(position);
            Intent intent = new Intent(activity, ScdjActivityF.class);
            intent.putExtra("ddbh", ddxzListItemEntity.getDdbh());
            intent.putExtra("cpbh", ddxzListItemEntity.getCpbh());
            intent.putExtra("cptp", ddxzListItemEntity.getCptp());
...
//                    break;
    ...
    public View generateDatePick3(String strSelDate) {
        ...
        final WheelView wheelYear = (WheelView) cardTypePopupView.findViewById(R.id.sel_year) ;
...
//        wheelHour.setCurrentItem(index);
        ...
        OnClickListener onClickDatePick = new OnClickListener() {
